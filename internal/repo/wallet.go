...
func (r *WalletRepo) GetUserCards(ctx context.Context, userID string) ([]model.UserCard, error) {
	rows, err := r.db.Query(ctx, `
		SELECT
			...
			uc.nickname, uc.points_expiry_date, uc.date_opened,
			uc.has_annual_fee, uc.custom_annual_fee,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&uc.Nickname, &uc.PointsExpiryDate, &uc.DateOpened,
			&uc.HasAnnualFee, &uc.CustomAnnualFee,
...
func (r *WalletRepo) UpdateCardDetails(ctx context.Context, userID, cardID string, req model.UpdateCardDetailsRequest) error {
	...
	if req.PointBalance != nil {
		setClauses = append(setClauses, fmt.Sprintf("point_balance = $%d", argNum))
		args = append(args, *req.PointBalance)
		argNum++
	}
	if req.Nickname != nil {
		setClauses = append(setClauses, fmt.Sprintf("nickname = $%d", argNum))
		args = append(args, *req.Nickname)
		argNum++
	}
	if req.PointsExpiryDate != nil {
		setClauses = append(setClauses, fmt.Sprintf("points_expiry_date = $%d", argNum))
		if *req.PointsExpiryDate == "" {
			args = append(args, nil)
		} else {
			args = append(args, *req.PointsExpiryDate)
		}
		argNum++
	}
	if req.DateOpened != nil {
		setClauses = append(setClauses, fmt.Sprintf("date_opened = $%d", argNum))
		if *req.DateOpened == "" {
			args = append(args, nil)
		} else {
			args = append(args, *req.DateOpened)
		}
		argNum++
	}
	if req.HasAnnualFee != nil {
		setClauses = append(setClauses, fmt.Sprintf("has_annual_fee = $%d", argNum))
		args = append(args, *req.HasAnnualFee)
		argNum++
	}
	if req.CustomAnnualFee != nil {
		setClauses = append(setClauses, fmt.Sprintf("custom_annual_fee = $%d", argNum))
		args = append(args, *req.CustomAnnualFee)
		argNum++
	}
	...
	query += fmt.Sprintf(" WHERE user_id = $%d AND card_id = $%d", argNum, argNum+1)
	args = append(args, userID, cardID)
