...
export function RecommendationCard({ rec, rank }: Props) {
  ...
  return (
    <div
      className="relative rounded-2xl p-5 lift"
      style={{
        background: isTop
          ? "linear-gradient(135deg, rgba(200,16,46,0.08) 0%, rgba(155,13,35,0.04) 100%)"
          : "var(--bg-elevated)",
        border: isTop
          ? "1px solid rgba(200,16,46,0.25)"
          : `1px solid var(--border-dim)`,
        boxShadow: isTop
          ? "0 4px 24px rgba(200,16,46,0.12), 0 1px 3px rgba(0,0,0,0.4)"
          : "0 1px 3px rgba(0,0,0,0.3)",
      }}
    >
      {/* Top highlight line for #1 */}
      {isTop && (
        <div className="absolute top-0 left-6 right-6 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(200,16,46,0.6), transparent)" }} />
      )}

      <div className="flex items-start gap-4">
        {/* Rank bubble */}
        <div
          className="w-8 h-8 rounded-xl shrink-0 flex items-center justify-center text-[13px] font-bold"
          style={{
            background: isTop
              ? "linear-gradient(135deg,#C8102E,#9B0D23)"
              : isSecond
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            ...
            {/* Effective return — the hero number */}
            <div className="text-right shrink-0">
              <div
                className="text-[22px] font-bold tracking-tight leading-none"
                style={{ color: isTop ? "#E8173A" : "var(--text-primary)" }}
              >
                {fmtPct(rec.effective_return)}
              </div>
              <div className="label-xs mt-1" style={{ color: "var(--text-tertiary)" }}>
                return
              </div>
            </div>
          </div>
          ...
          {/* Stats row */}
          <div
            className="mt-4 grid grid-cols-3 gap-3 pt-3.5 rounded-xl px-3 pb-3"
          ...
          >
            ...
            <div>
              <div
                ...
                style={{ color: isTop ? "#4ADE80" : "var(--text-primary)" }}
              >
          ...
          <div className="flex items-center gap-2 mt-2.5 flex-wrap">
            ...
            {isTop && (
              <span
                className="label-xs px-2 py-0.5 rounded-full ml-auto"
                style={{ background: "rgba(200,16,46,0.12)", color: "#E8173A", border: "1px solid rgba(200,16,46,0.2)" }}
              >
                Best value
              </span>
            )}
