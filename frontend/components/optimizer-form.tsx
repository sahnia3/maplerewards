...
import { listCategories, optimize, ensureSession, addSpendEntry } from "@/lib/api";
...
export function OptimizerForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null); setLoading(true); setLogged(false);
    try {
  ...
  function handleLog(rec: CardRecommendation) {
    const cat = categories.find(c => c.slug === categorySlug);
    addSpendEntry({
      id: crypto.randomUUID(),
      date: new Date().toISOString(),
      category_slug: categorySlug,
      category_name: cat?.name ?? categorySlug,
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-4 items-end">
            {/* Category */}
            <div>
              <label className="label-xs mb-2.5 block" style={{ color: "var(--text-tertiary)" }}>Category</label>
              <div className="relative">
                <select
                  value={categorySlug} onChange={e => setCategorySlug(e.target.value)} disabled={catLoading}
                  className="w-full h-11 pl-10 pr-4 rounded-xl text-[14px] font-medium appearance-none cursor-pointer outline-none transition-all"
                  style={{ background: "rgba(255,255,255,0.05)", border: "1px solid var(--border-mid)", color: categorySlug ? "var(--text-primary)" : "var(--text-tertiary)", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.3)" }}
                ...
                >
                  <option value="" disabled style={{ background: "var(--bg-overlay)" }}>{catLoading ? "Loading…" : "Select a category"}</option>
                  {categories.map(cat => (
            ...
            {/* Amount */}
            <div>
              <label className="label-xs mb-2.5 block" style={{ color: "var(--text-tertiary)" }}>Spend amount</label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[14px] font-medium pointer-events-none select-none" style={{ color: "var(--text-tertiary)" }}>$</span>
                <input
                  ...
                  style={{ background: "rgba(255,255,255,0.05)", border: "1px solid var(--border-mid)", color: "var(--text-primary)", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.3)" }}
                  onFocus={e => { e.currentTarget.style.borderColor = "rgba(200,16,46,0.5)"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(200,16,46,0.12), inset 0 1px 2px rgba(0,0,0,0.3)"; }}
                  onBlur={e => { e.currentTarget.style.borderColor = "var(--border-mid)"; e.currentTarget.style.boxShadow = "inset 0 1px 2px rgba(0,0,0,0.3)"; }}
                />
                <span className="absolute right-3.5 top-1/2 -translate-y-1/2 label-xs pointer-events-none" style={{ color: "var(--text-tertiary)" }}>CAD</span>
              </div>
            </div>

            {/* CTA */}
            <button type="submit" disabled={loading || !categorySlug || !spendAmount}
        ...
        <div className="mt-6">
          {results.length === 0 ? (
            <div className="rounded-2xl p-10 text-center" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-dim)" }}>
              ...
              <p className="text-[14px]" style={{ color: "var(--text-secondary)" }}>No cards found for this category.</p>
              <a href="/wallet" className="inline-block mt-4 text-[13px] font-medium text-[#C8102E] hover:underline">Add cards to your wallet →</a>
            </div>
          ) : (
            <>
              <div className="flex items-baseline justify-between mb-4 px-1">
                <p className="text-[13px] font-semibold text-white">{results.length} card{results.length !== 1 ? "s" : ""} ranked</p>
                <p className="text-[12px]" style={{ color: "var(--text-tertiary)" }}>for ${parseFloat(spendAmount).toFixed(2)} CAD</p>
              </div>
              <div className="flex flex-col gap-3">
                {results.map((rec, i) => (
                  <RecommendationCard key={rec.card_id} rec={rec} rank={i + 1} onLog={i === 0 ? () => handleLog(rec) : undefined} logged={logged && i === 0} />
                ))}
              </div>
              <p className="text-center text-[12px] mt-5" style={{ color: "var(--text-tertiary)" }}>
                Tap &ldquo;Log spend&rdquo; on the best card to track it in{" "}
                <a href="/insights" className="text-[#C8102E] hover:underline">Insights</a>
              </p>
            </>
          )}
        </div>
