...
- **P2.6 REMAINING — NOT a dead-button wire.** Credits are definition-driven (`card_credit_defs`; only GET list + POST redeem endpoints, NO create). Root cause = **sparse seed: 9 defs / 5 of 104 cards** (same class as P0.4 card_value_components 6/104). The empty calendar is a data-coverage gap, not the "Add cards" link. Two valid fixes, both LARGE: (a) per-card credit-definition seed-curation migration (research each card's annual credits — Amex Plat $200 travel/$200 dining, etc. — for the ~50 cards that have credits; cap-remediation-scale research), or (b) a new self-log vertical (card_credit_defs INSERT endpoint + form + per-user override). Decide (a) vs (b) before building. Tables: `card_credit_defs`, `user_card_credits` (mig 000010).
- **P4.1 stack suggester REMAINING:** `/stack-recommend` endpoint exists but `StackService.Recommend` returns canned templates; `frontend/components/stack-templates.tsx` 4 static. Need profile inference from the user's logged spend (reuse `OptimizerService.GetBestCard` per category) → recommend a stack + reasoning. Logic-heavy, no migration.
